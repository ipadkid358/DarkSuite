...
// RoutingSheet (AirPlay options) looks like a UIAlertStyleSheet but isn't
%hook MPAVRoutingSheet

- (void)layoutSubviews {
    %orig;
    
    UIView *cancelButton = [self valueForKey:@"_cancelButton"];
    cancelButton.backgroundColor = UIColor.blackColor;
    
    UIView *controlView = [self valueForKey:@"_controlsView"];
    UIView *containerView = controlView.subviews[1];
    UIView *tableView = containerView.subviews.firstObject;
    tableView.backgroundColor = UIColor.blackColor;
    
    for (UIView *headerView in tableView.subviews) {
        if (headerView.class == %c(MPAVRoutingTableHeaderView)) {
            headerView.backgroundColor = UIColor.blackColor;
            for (CALayer *shapeLayer in headerView.layer.sublayers) {
                if (shapeLayer.class == CAShapeLayer.class) {
                    [shapeLayer removeFromSuperlayer];
                }
            }
            
            for (UILabel *headerLabel in headerView.subviews) {
                if ([headerLabel respondsToSelector:@selector(setTextColor:)]) {
